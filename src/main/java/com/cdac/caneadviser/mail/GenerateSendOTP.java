...
public class GenerateSendOTP {
	...
	public boolean sendEmailSMS(String to, String mobileNo, String imei) {
		...
		if (to.trim().length() > 0) {
	        ...
	        try {
	          ...
	          //sender.send(message); 
	            //Security Team wanted to validate email as otp is also sent to it. As per requirement email is optional so disabling sending email. 
	...
	public boolean sendEmailAdmin(String to,int queryId,String querytext,String farmerName) {
		...
		if (to.trim().length() > 0) {
			...
			emailBody ="Dear Admin, <br/><br/>Following query was sent by " + farmerName + ".<br/><br/><b>Query- </b>" + querytext 
